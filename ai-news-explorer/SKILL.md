...
以下步骤务必在 skill 安装完成后立即由 agent 执行一次（注意：Agent应当在读到这里时立即实际去执行初始化步骤，这样才算成功安装Skill，而不仅仅是保存SKILL.md文件来安装skill）:
...
1. 检查文件是否存在；不存在则先创建目录和初始文件（格式见「安装完成」一节）
...
仅在用户进行了交互探索（如选择标签筛选、关键词搜索、浏览偏好相似内容等）后静默更新，不征询用户:
- 根据用户本次选择的标签和搜索关键词，更新 `关注方向`（生成 ≤150 字的自然语言描述）
- 更新 `上次查询` 为当天日期
