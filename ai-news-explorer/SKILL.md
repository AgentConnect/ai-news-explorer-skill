...
在Skill安装完成后立即向用户展示以下提示:
> 安装完成！如果你需要，可以和 Agent 说：每天 09:00 自动通过 ai-news-explorer 拉取最新 AI 资讯。
...
- 注意：无论是首屏输出还是后续交互探索追问中的输出，要在对应的新闻条目中展示原始数据中的链接（注意是“链接”这个字段的URL而不是“内容详情”字段中的URL），方便用户自行浏览，而不只是展示新闻的文本标题和摘要。
...
如 `关注方向` 有非空内容且 `上次查询` ≤7 天，追加:
```
4. 📌 查看与上次关注方向相似的内容 — 上次: {偏好摘要}
```

如 `关注方向` 有非空内容但 `上次查询` >7 天，追加:
```
...
1. 检查文件是否存在；不存在则先创建目录和初始文件:
   ```bash
   mkdir -p "${XDG_CONFIG_HOME:-$HOME/.config}/ai-news/"
   ```
   写入初始内容到 `${XDG_CONFIG_HOME:-~/.config}/ai-news/memory.md`:
   ```markdown
   # AI News 阅读偏好
   ...
   ```
   并告知用户:
   > 你的阅读偏好会保存在 `~/.config/ai-news/memory.md`，可以随时手动编辑或删除。
2. 读取并解析 `关注方向:` 后的文本和 `上次查询:` 后的日期
3. 如 `关注方向` 为空，视为无偏好数据，首屏导航菜单不展示偏好选项
4. 如 `上次查询` 有值，计算距今天数，用于判断是否过期（>7 天）

### 写入

在用户对资讯产生任何进一步地追问、筛选、搜索等可以展现出用户任何偏好取向的态度信息时都静默更新，不征询用户:
- 根据用户展现出的偏好取向来更新 `关注方向`（生成 ≤150 字的自然语言描述）
- 更新 `上次查询` 为当天日期
